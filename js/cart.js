...
function addToCart(productData) {
  ...
  const uniqueId = productData.id || `product-${productData.name.replace(/\s+/g, '-').toLowerCase()}`;
  
  // Check if item already exists
  const existingItemIndex = window.cart.findIndex(item => item.id === uniqueId);
  
  if (existingItemIndex > -1) {
    // Item exists, increase quantity
    window.cart[existingItemIndex].quantity += productData.quantity || 1;
    console.log('✓ Item quantity increased to:', window.cart[existingItemIndex].quantity);
  } else {
...
function initializeCartButtons() {
  ...
  document.querySelectorAll('.product-card .hover-cart button:not([data-cart-initialized])').forEach((btn) => {
  ...
  document.querySelectorAll('.offer-card .overlay button:not([data-cart-initialized])').forEach((btn) => {
    btn.setAttribute('data-cart-initialized', 'true');
    btn.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      ...
      const productName = card.querySelector('.offer-info h3')?.textContent?.trim() || 'Product';
      
      // Get the price from .price-row structure
      const priceUsdEl = card.querySelector('.offer-info .price .price-row .price-usd');
      const priceRielEl = card.querySelector('.offer-info .price .price-row .price-riel');
      ...
      if (priceUsdEl && priceRielEl) {
        ...
        // Fallback
        const priceText = card.querySelector('.offer-info .price')?.textContent || '0';
        priceData = extractPrice(priceText);
      }
  ...
  productPageButtons.forEach((btn) => {
    ...
    btn.addEventListener('click', function(e) {
      ...
      const priceUsdEl = card.querySelector('.price-usd, .current-price, .product-price .price-usd');
      const priceRielEl = card.querySelector('.price-riel, .product-price .price-riel');
      ...
      if (priceUsdEl && priceRielEl) {
        ...
        const priceContainer = card.querySelector('.product-price, .price, .product-details .price');
        if (priceContainer) {
  ...
  console.log('✓ Cart buttons initialized. Total buttons:', 
...
const observer = new MutationObserver(function(mutations) {
  mutations.forEach(function(mutation) {
    if (mutation.addedNodes.length) {
      const isProductGrid = mutation.target.id === 'productGrid' || 
...
function initCartSidebar() {
  ...
  const checkElements = () => {
    const cartIcon = document.getElementById('cartIconBtn') || document.querySelector('.cart-icon');
    ...
    const retryInterval = setInterval(() => {
      ...
      if (checkElements() || attempts >= maxAttempts) {
        ...
        if (attempts >= maxAttempts) {
          console.error('Failed to initialize cart sidebar after maximum attempts');
        }
      }
    }, 200);
  }
